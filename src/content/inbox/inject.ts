...
import InboxViewer from './InboxViewer.svelte';
import tailwindCss from '../../styles/tailwind.css?inline';

const HOST_ID = 'paintbrush-inbox-root';
...
export function mountInboxViewer(): () => void {
  ...
  host.id = HOST_ID;
  host.style.cssText = 'all: initial; display: block; position: fixed; top: 0; left: var(--pb-left-inset, 84px); right: var(--pb-sidebar-w, 340px); bottom: 0; overflow: hidden; background: transparent; z-index: 1; transition: right 300ms cubic-bezier(0.22,0.61,0.36,1); pointer-events: auto;';
  ...
  const app = mount(InboxViewer, { target: appRoot });
